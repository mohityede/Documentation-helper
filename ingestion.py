...
embeddings = GoogleGenerativeAIEmbeddings(
    model="gemini-embedding-001", output_dimensionality=2048
)
...
async def index_docs_async(documents: List[Document], batch_size: int = 50):
    ...
    if success == len(batches):
        logger.log_success(
            f"VectorStore Indexing: All batches processed successfully! ({success}/{len(batches)})"
        )
    else:
        logger.log_warning(
            f"VectorStore Indexing: Processed {success}/{len(batches)} batches successfully"
        )
...
async def main():
    ...
    all_docs = [
        ...
        for res in crawl_result["results"]
    ]
    logger.log_success(
        f"TavilyCrawl: successfully crawled {len(all_docs)} URLs from documantation site"
    )
